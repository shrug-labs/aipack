...
func TestAgentFrontmatter_ValidateRefs_OK(t *testing.T) {
	...
	ws := fm.ValidateRefs(servers, skills)
	if len(ws) != 0 {
		t.Fatalf("expected no warnings, got %v", ws)
	}
}
...
func TestSkillFrontmatter_Validate_OK(t *testing.T) {
	t.Parallel()
	fm := SkillFrontmatter{Name: "my-skill", Description: "does things"}
	ws := fm.Validate("my-skill")
	if len(ws) != 0 {
		t.Fatalf("expected no warnings, got %v", ws)
	}
