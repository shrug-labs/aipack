...
func (m packsModel) applyProgress(e app.PackUpdateEvent) (packsModel, tea.Cmd) {
	if e.Pack == "" {
		return m, nil
	}
...
func (m packsModel) applyRowClear(msg packRowClearMsg) packsModel {
...
func packsBatchStatus(m packsModel) string {
	total := m.batchTotal
	if total <= 0 {
...
func packsInstallStatus(m packsModel) string {
...
// row decoration.
func (m packsModel) applyCancellation(results []app.PackUpdateResult) packsModel {
	for _, r := range results {
...
func (m packsModel) reconcileFromResults(results []app.PackUpdateResult) (packsModel, []tea.Cmd) {
	var cmds []tea.Cmd
	for _, r := range results {
		if isCancelledResult(r) {
